        ...
        <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between flex-wrap gap-3">
              <div>
                <p className="text-sm font-semibold text-slate-800">Youth Rankings</p>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  ITTF World Youth Rankings · updated weekly
                  {youthRows[0]?.publish_date ? ` · ${new Date(youthRows[0].publish_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}` : ''}
                </p>
              </div>
              {/* Age group selector */}
              <div className="flex items-center gap-0.5 bg-slate-100 rounded-lg p-0.5">
                {['U13', 'U15', 'U17', 'U19'].map(ag => (
                  <button key={ag} onClick={() => setYouthAgeGroup(ag)}
                    className={`px-2.5 py-1 rounded-md text-xs font-bold transition-all ${
                      youthAgeGroup === ag ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                    {ag}
                  </button>
                ))}
              </div>
            ...
            {youthLoading ? (
              ...
              <div className="py-10 text-center text-slate-400 text-sm">No data for {youthAgeGroup} {YOUTH_DISCIPLINES.find(d => d.code === youthDiscipline)?.label}</div>
            ) : (() => {
              ...
              return (
                <div className="overflow-x-auto">
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #f1f5f9', color: '#94a3b8', textTransform: 'uppercase', fontSize: 10, letterSpacing: '0.05em' }}>
                        <th style={{ textAlign: 'right', padding: '8px 8px 8px 16px', width: 40 }}>#</th>
                        <th style={{ textAlign: 'left', padding: '8px 8px' }}>
                          {isDoubles ? 'Pair' : 'Player'}
                        </th>
                        <th style={{ textAlign: 'left', padding: '8px 8px' }}>Country</th>
                        <th style={{ textAlign: 'right', padding: '8px 8px' }}>Points</th>
                        <th style={{ textAlign: 'right', padding: '8px 16px 8px 8px' }}>Change</th>
                      </tr>
                    </thead>
                    <tbody>
                      {youthRows.map((row, i) => {
                        const isIndia = isDoubles
                          ? row.country_code1 === 'IND' || row.country_code2 === 'IND'
                          : row.country_code === 'IND';
                        const diff = row.rank_diff != null ? parseInt(row.rank_diff) : null;
                        return (
                          <tr key={isDoubles ? row.pair_id + row.age_category : row.ittf_id + row.age_category}
                            style={{
                            ...
                            <td style={{ padding: '6px 8px', color: '#64748b', fontSize: 11 }}>
                              {isDoubles
                                ? (row.country_code1 === row.country_code2 ? row.country_code1 : `${row.country_code1}/${row.country_code2}`)
                                : row.country_code}
                            </td>
                            <td style={{ textAlign: 'right', padding: '6px 8px', color: '#64748b', fontVariantNumeric: 'tabular-nums' }}>
                              {isDoubles ? (row.points != null ? Math.round(row.points).toLocaleString() : '—') : (row.points_ytd != null ? Math.round(row.points_ytd).toLocaleString() : '—')}
                            </td>
                            <td style={{ textAlign: 'right', padding: '6px 16px 6px 8px', fontWeight: 500,
                              color: diff == null || diff === 0 ? '#94a3b8' : diff < 0 ? '#10b981' : '#ef4444',
                              fontVariantNumeric: 'tabular-nums' }}>
                              {diff == null || diff === 0 ? '—' : diff < 0 ? `▲${Math.abs(diff)}` : `▼${diff}`}
                            </td>
                          </tr>
                        );
                      })}
